...
class Hydralisk(object):
    ...
    @staticmethod
    def get_schedule(schedule_conf):
        if isinstance(schedule_conf, str):
            cron_list = schedule_conf.split(' ')
            ...
            return crontab(minute=minute, hour=hour,
                           ...
                           month_of_year=month_of_year), None, None
        elif isinstance(schedule_conf, int):
            return timedelta(seconds=int(schedule_conf)), None, None
        elif isinstance(schedule_conf, tuple):
            schedule = Hydralisk.get_schedule(schedule_conf[0])[0]
            # now only support return bool function
            assert inspect.isfunction(schedule_conf[1])
            callback = None
            if len(schedule_conf) > 2 and inspect.isfunction(schedule_conf[2]):
                callback = schedule_conf[2]

            return schedule, schedule_conf[1], callback
        else:
            raise ValueError('schedule format wrong!!!!')
    ...
    def init_schedule(self):
        ...
        for view_name, view_dict in self.view_conf.items():
            ...
            schedule, condition, callback =\
                Hydralisk.get_schedule(view_dict['schedule'])
        ...
        def cron_chart_job(view_name, chart_conf, mail_template, mail_to,
                           condition=None, callback=None):
            if condition and not condition():
                _LOGGER.info('cron condition not satisfied!, Quit!')
                return
            try:
